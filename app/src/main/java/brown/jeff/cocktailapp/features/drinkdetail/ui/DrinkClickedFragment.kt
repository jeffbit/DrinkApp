package brown.jeff.cocktailapp.features.drinkdetail.ui
...
import brown.jeff.cocktailapp.ui.drinkclicked.DrinkClickedFragmentArgs
import brown.jeff.cocktailapp.ui.drinkclicked.DrinkClickedFragmentDirections
...
class DrinkClickedFragment : Fragment() {
    ...
    private fun observeRelatedDrinks() {
        drinkClickedViewModel.clickedDrink.observe(viewLifecycleOwner, Observer { drink ->
            drinkClickedViewModel.getDrinksByIngredient(drink.ingredients[0])
            favoriteFloatingActionButton(requireView(), drink.id)
    ...
    private fun shareDrinkUrl(activity: Activity) {
        sharedrink_button.setOnClickListener {
            drinkClickedViewModel.clickedDrink.observe(viewLifecycleOwner, Observer { drink ->
                shareDrink(activity, getString(R.string.drink_url, drink.id, drink.title))
            })
        }
    }
