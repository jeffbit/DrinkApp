...
class SearchRecentDrinksViewModel(private val drinkRepositoryImpl: DrinkRepositoryImpl) : ViewModel() {
    private val recentDrinks = MutableLiveData<List<DrinkEntity>>()
    ...
    private val _currentDrinks = MutableLiveData<List<DrinkEntity>>()
    ...
    fun getRecentDrinks() {
        ...
        viewModelScope.launch {
            when (val result = drinkRepositoryImpl.getPopularDrinks()) {
                is Success -> {
                    recentDrinks.postValue(result.data.drinkEntities)
                    _loadingDrinks.postValue(false)
                    _currentDrinks.value = result.data.drinkEntities
                }
                is Result.Failure -> {
